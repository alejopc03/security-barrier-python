...
class IEModel:
    def __init__(self, model_xml, model_bin, ie_core, target_device, num_requests, labels=None, batch_size=1, prob_threshold=0.5):
        ...
        self.output_names = [name for name in self.net.outputs]
        self.input_shape = self.net.inputs[self.input_name].shape
        self.input_n, self.input_c, self.input_h, self.input_w = self.input_shape[0], self.input_shape[1], self.input_shape[2], self.input_shape[3]
        self.output_shapes = [output.shape for name, output in self.net.outputs.items()]
        self.num_requests = num_requests
        self.labels = labels
    ...
    def wait_request(self, req_id):
        ...
        infer_result = self.exec_net.requests[req_id].outputs
        result = {}
        for output_name in self.output_names:
            result[output_name] = infer_result[output_name]
